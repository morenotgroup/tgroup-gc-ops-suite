...
export default function FinanceClient({ role }: { role: Role }) {
  ...
  useEffect(() => { if(company) load(); }, [company, comp]);
    ...
    <main style={{ maxWidth: 1100, margin: "0 auto", padding: 24 }}>
      <GlassCard>
            ...
            <thead>
              <tr>
                {["Nome","Valor Esperado","NF(planilha)","Link(planilha)","Status"].map(h=>(
                  <th key={h} style={{ textAlign:"left", padding:"10px 8px", borderBottom:"1px solid rgba(255,255,255,.12)", opacity:.85 }}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
        ...
        <p style={{ marginTop: 12, opacity: 0.7, fontSize: 12 }}>
          Dica: peça para GC rodar “Rodar Agora” no último dia do fechamento para garantir que a aba FIN_* está atualizada.
        </p>
      </GlassCard>
    </main>
  );
